...
class UploadedFile(UserCreatedBase):
    """
    File attachment stored in DB or filesystem.

    Stores metadata (filename, size, mime type) and either binary data (small
    files) or a filesystem path (large files). Used by VirtualAttachmentReference
    subclasses to attach files to comments, events, etc.
    """
    ...
    is_technical_library = db.Column(db.Boolean, nullable=False, default=False)
    ...
    @classmethod
    def generate_file_path(cls, row_id, filename, is_technical_library=False):
        """Generate filesystem path for large attachments using row_id_filename structure"""
        timestamp = datetime.now()
        safe_filename = secure_filename(filename)
        base_dir = "instance/technical_library" if is_technical_library else "instance/large_attachments"
        return f"{base_dir}/{timestamp.year}/{timestamp.month:02d}/{row_id}_{safe_filename}"
    ...
    def save_file(self, file_data, filename):
        ...
        if self.storage_type == 'database':
            ...
            self.file_path = self.generate_file_path(self.id, filename, is_technical_library=self.is_technical_library)
            self.file_data = None
...
class VirtualAttachmentReference(UserCreatedBase):
    """
    Abstract base for linking an Attachment to another entity (comment, event, etc.).

    Uses a global all_attachment_references_id sequence. Subclasses define
    attached_to_id and attached_to_type; display_order and attachment_type
