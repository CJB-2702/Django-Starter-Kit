...
Related: [STANDARDS.md](../ARCHITECTURE/STANDARDS.md), [HTMX_PATTERNS.md](../ARCHITECTURE/HTMX_PATTERNS.md), [ENDPOINT_PATTERNS.md](../ARCHITECTURE/ENDPOINT_PATTERNS.md), [REQUIREMENTS.md](../REQUIREMENTS.md), [VISION.md](../VISION.md).

---

## Principles

- Use **Bulma** for layout and components, consistent with [STANDARDS.md](../ARCHITECTURE/STANDARDS.md) and vendored assets under [bulma-1.0.4/](../../bulma-1.0.4/).
...
- Prefer a **server-rendered multi-page application (MPA)** that feels like **one coherent product**: standard links and forms, shared layout, and **Django `request.session`** for transient UI state so a **full reload** returns the user to the **same step and data** (especially in [guided flows](#multi-step-flows)). This is not the same as mounting unrelated SPAs; it means predictable server HTML and session-backed continuity. Details: [HTMX_PATTERNS.md](../ARCHITECTURE/HTMX_PATTERNS.md) §1.
...
Breadcrumbs may live in a shared block extended from [templates/base.html](../../app/templates/base.html) or an include; keep markup consistent across apps.
...
Collection URLs follow the **plural resource** pattern in [ENDPOINT_PATTERNS.md](../ARCHITECTURE/ENDPOINT_PATTERNS.md) (for example `…/events-application/events`). **Singular resource** URLs follow the detail pattern (for example `…/events-application/event/<id>`). Use the same **`template fragments and visual format`** query vocabulary for **both**: list and detail should **look like the same design system at the same density**—for example `…/events?format=medium` and `…/event/<id>?format=medium` share typography, spacing, and information density conventions for that resource type.
...
**HTMX / fragment `format` values** (same query parameter name, different contract) are defined in [ENDPOINT_PATTERNS.md](../ARCHITECTURE/ENDPOINT_PATTERNS.md) §3.6 and [HTMX_PATTERNS.md](../ARCHITECTURE/HTMX_PATTERNS.md)—for example `format=htmx-search-results`, `format=htmx-focused`.
...
| :--- | :--- | :--- |
| **List density** | `condensed` / `medium` / `large` | Not used—use `hx-select` on a full page when possible ([HTMX_PATTERNS.md](../ARCHITECTURE/HTMX_PATTERNS.md) §6). |
...
- When a widget is “just another view” of an existing resource, prefer the **canonical URL + `format=`** pattern from [ENDPOINT_PATTERNS.md](../ARCHITECTURE/ENDPOINT_PATTERNS.md) §3.6 so the same addressable resource does not sprawl across duplicate paths. If a widget grows too complex for query parameters alone, see [Custom formats and dedicated fragment paths](#custom-formats-and-dedicated-fragment-paths).
- See [HTMX_PATTERNS.md](../ARCHITECTURE/HTMX_PATTERNS.md) §7 for focused widgets and fragment rules.
...
- Persist step and draft field state in **session** (namespaced), aligned with [HTMX_PATTERNS.md](../ARCHITECTURE/HTMX_PATTERNS.md) §2.C.
...
- **Layer HTMX** after the baseline works, following [HTMX_PATTERNS.md](../ARCHITECTURE/HTMX_PATTERNS.md) (for example `hx-select` on full pages, fragment exceptions where documented).
- **Exceptions** where HTMX is expected even during initial design:
  ...
  - **Fields with more than eight options** where a typeahead, searchable dropdown, or lazy-loaded select improves usability—use HTMX as in [HTMX_PATTERNS.md](../ARCHITECTURE/HTMX_PATTERNS.md) §3.
...
- **Duplicate routes** for list density or HTMX variants—use **one collection URL** and branch on `format` ([ENDPOINT_PATTERNS.md](../ARCHITECTURE/ENDPOINT_PATTERNS.md) §3.6); same idea for singular URLs before adding `…/fragments/…`.
...
- [HTMX_PATTERNS.md](../ARCHITECTURE/HTMX_PATTERNS.md) — Dynamic behavior, session drafts, search exception, CSRF.
- [ENDPOINT_PATTERNS.md](../ARCHITECTURE/ENDPOINT_PATTERNS.md) — Routes, collections, `format` for partials.
